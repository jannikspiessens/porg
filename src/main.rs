use clap::{Parser, error::Error, error::ErrorKind, CommandFactory};
use url::Url;
use reqwest::Client;
...
async fn get_info(url_err: Error, client: &Client, url: &mut Url) 
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (cryptobib_name, eprint_id) = get_info(url_err, &client, &mut url).await?;
    ...
    if !path.is_file() {
        ...
        url.set_path(format!("{eprint_id}.pdf").as_str());
